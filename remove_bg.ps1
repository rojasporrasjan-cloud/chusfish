...
public class ImageProcessor {
    public static void RemoveBackground(string inputPath, string outputPath, int tolerance) {
        using (Bitmap bmp = new Bitmap(inputPath)) {
            // Convert to 32bppArgb to support transparency
            ...
            while (q.Count > 0) {
                ...
                // If within tolerance, make transparent and enqueue neighbors
...
foreach ($file in $files) {
    ...
    [ImageProcessor]::RemoveBackground($file.FullName, $outFile, 60)
}
